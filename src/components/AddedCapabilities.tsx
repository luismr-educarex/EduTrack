...
import {
  CalendarEvent, ClassGroup, ContentItem, GradingScale, Module, calendarEventService,
...
export function CourseManagementFeature() {
  ...
  const [moduleForm, setModuleForm] = useState({ code: '', name: '', cycle: '', course: new Date().getFullYear() + '–' + (new Date().getFullYear() + 1), deliveryMode: 'in_person' as Module['deliveryMode'] });
  ...
  const addModule = async () => {
    if (!moduleForm.code.trim() || !moduleForm.name.trim() || !moduleForm.cycle.trim()) return toast.error('Código, nombre y ciclo son obligatorios');
    const id = `module-${crypto.randomUUID()}`;
    setSavingModuleId(id);
    try {
      await moduleService.upsert({ id, ...moduleForm, evaluationCount: 2, totalStudents: 0 });
      setActiveModuleId(id);
      toast.success('Módulo creado');
      window.location.reload();
    } catch (cause) {
      toast.error(cause instanceof Error ? cause.message : 'No se pudo crear el módulo');
      setSavingModuleId(null);
    }
  };
  const changeModuleDeliveryMode = async (module: Module, deliveryMode: Module['deliveryMode']) => {
    if (deliveryMode === module.deliveryMode) return;
    setSavingModuleId(module.id);
    try {
      await moduleService.upsert({ ...module, deliveryMode });
      const labels: Record<Module['deliveryMode'], string> = { in_person: 'presencial', online: 'online', intermodular: 'proyecto intermodular' };
      toast.success(`Módulo cambiado a ${labels[deliveryMode]}`);
      window.location.reload();
    } catch (cause) {
      toast.error(cause instanceof Error ? cause.message : 'No se pudo cambiar la modalidad');
      setSavingModuleId(null);
    }
  };
  ...
  return <div className="space-y-5"><div className="flex gap-2 border-b border-border">{tabs.map(item => <button key={item.id} onClick={() => setTab(item.id)} className={`px-4 py-2 text-sm border-b-2 ${tab === item.id ? 'border-primary text-primary font-semibold' : 'border-transparent text-muted-foreground'}`}>{item.label}</button>)}</div>
    {tab === 'modules' && <Panel title="Módulos formativos" description="El tipo de módulo activa el flujo de EduTrack, EduCodeCheck o EduProyectosCheck."><div className="space-y-2 mb-5">{modules.map(module => <div key={module.id} className={`border rounded-lg p-3 flex items-center gap-3 ${module.id === activeModule?.id ? 'border-primary bg-primary/5' : 'border-border'}`}>{module.deliveryMode === 'online' ? <Laptop size={17} className="text-primary" /> : <School size={17} className="text-primary" />}<button className="flex-1 text-left" onClick={() => setActiveModuleId(module.id)}><p className="text-sm font-semibold">{module.code} · {module.name}</p><p className="text-xs text-muted-foreground">{module.cycle} · {module.course} · {module.deliveryMode === 'online' ? 'Online' : module.deliveryMode === 'intermodular' ? 'Proyecto intermodular' : 'Presencial'} · {module.totalStudents} alumnos</p></button><select className="border border-border rounded-md bg-background px-2 py-1.5 text-xs disabled:opacity-50" value={module.deliveryMode} disabled={savingModuleId === module.id} onChange={e => changeModuleDeliveryMode(module, e.target.value as Module['deliveryMode'])} aria-label={`Tipo de ${module.code}`}><option value="in_person">Presencial · EduTrack</option><option value="online">Online · EduCodeCheck</option><option value="intermodular">Proyecto intermodular · EduProyectosCheck</option></select><button onClick={() => removeModule(module.id)}><Trash2 size={15} className="text-danger" /></button></div>)}</div><div className="grid sm:grid-cols-2 gap-3"><input className={inputClass} placeholder="Código" value={moduleForm.code} onChange={e => setModuleForm(form => ({ ...form, code: e.target.value }))} /><input className={inputClass} placeholder="Nombre" value={moduleForm.name} onChange={e => setModuleForm(form => ({ ...form, name: e.target.value }))} /><input className={inputClass} placeholder="Ciclo y curso" value={moduleForm.cycle} onChange={e => setModuleForm(form => ({ ...form, cycle: e.target.value }))} /><input className={inputClass} placeholder="Curso académico" value={moduleForm.course} onChange={e => setModuleForm(form => ({ ...form, course: e.target.value }))} /><select className={inputClass} value={moduleForm.deliveryMode} onChange={e => setModuleForm(form => ({ ...form, deliveryMode: e.target.value as Module['deliveryMode'] }))}><option value="in_person">Presencial · EduTrack</option><option value="online">Online · EduCodeCheck</option><option value="intermodular">Proyecto intermodular · EduProyectosCheck</option></select></div><button className={`${primaryButton} mt-3`} disabled={savingModuleId !== null} onClick={addModule}><Plus size={16} />{savingModuleId ? 'Guardando…' : 'Crear módulo'}</button></Panel>}
