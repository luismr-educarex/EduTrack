...
export function CourseManagementFeature() {
  ...
  const [moduleForm, setModuleForm] = useState({ code: '', name: '', cycle: '', course: new Date().getFullYear() + '–' + (new Date().getFullYear() + 1), deliveryMode: 'in_person' as 'in_person' | 'online' });
  ...
  const changeModuleDeliveryMode = async (module: Module, deliveryMode: Module['deliveryMode']) => {
    ...
    try {
      ...
      toast.success(`Módulo cambiado a ${deliveryMode === 'online' ? 'online' : 'presencial'}`);
      window.location.reload();
    } catch (cause) {
