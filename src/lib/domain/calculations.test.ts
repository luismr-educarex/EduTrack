...
import {
  activityCriterionWeight,
  activityValuePercentage,
  criterionGrade,
  criterionStatus,
  evaluationGrade,
  moduleGrade,
...
describe('academic calculation engine', () => {
  ...
  it('derives activity weight and normalized value from criteria', () => {
    expect(activityCriterionWeight(activities[0], criteria)).toBe(3);
    expect(activityCriterionWeight(activities[1], criteria)).toBe(1);
    expect(activityValuePercentage('a-1', activities, criteria)).toBe(75);
    expect(activityValuePercentage('a-2', activities, criteria)).toBe(25);
  });

  it('calculates criterion, outcome, unit, evaluation and module grades consistently', () => {
    expect(criterionGrade('s-1', 'ce-1', activities, criteria, grades)).toBe(6.14);
    expect(outcomeGrade('s-1', 'ra-1', activities, criteria, grades)).toBe(6.9);
    expect(unitGrade('s-1', 'u-1', activities, criteria, grades)).toBe(6);
    expect(evaluationGrade('s-1', 'e-1', activities, criteria, grades)).toBe(6);
    expect(
      moduleGrade(
        's-1',
        [
