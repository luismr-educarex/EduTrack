...
export function difficultyPoints(difficulty: DomainCriterion['difficulty']): number {
  return difficulty === 'básico' ? 3 : difficulty === 'medio' ? 2 : 1;
}

export function weightedAverage(
  items: { value: GradeValue; weight: number }[],
  digits = 2
): GradeValue {
  ...
  return round(
    valid.reduce((sum, item) => sum + item.value * item.weight, 0) / totalWeight,
    digits
  );
}
...
export function activityGrade(
  ...
  return (
    grades.find((item) => item.studentId === studentId && item.activityId === activityId)?.grade ??
    null
  );
}

export function activityCriterionWeight(
...
export function activityValuePercentage(
  activityId: string,
  ...
  const siblings = activities.filter((item) =>
    activity.unitId
...
export function criterionGrade(
  studentId: string,
  criterionId: string,
  activities: DomainActivity[],
  criteria: DomainCriterion[],
  ...
  return weightedAverage(
    activities
      .filter((activity) => activity.ceIds.includes(criterionId))
      .map((activity) => ({
...
export function outcomeGrade(
  ...
  return weightedAverage(
    criteria
      .filter((criterion) => criterion.raId === outcomeId)
      .map((criterion) => ({
...
export function evaluationGrade(
  studentId: string,
  evaluationId: string,
  activities: DomainActivity[],
  criteria: DomainCriterion[],
  ...
  return weightedAverage(
    activities
      .filter((activity) => activity.evaluationId === evaluationId)
      .map((activity) => ({
        value: activityGrade(studentId, activity.id, grades),
        weight: activityCriterionWeight(activity, criteria) || activity.weight || 1,
      }))
